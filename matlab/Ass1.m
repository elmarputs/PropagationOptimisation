...
state_0_1 = dlmread(strcat(path, 'stateHistory_interpolated_1_0.dat'));
state_0_2 = dlmread(strcat(path, 'stateHistory_interpolated_2_0.dat'));

error_0_1 = state_0_1 - state_0_0;
error_0_2 = state_0_2 - state_0_0;

error_0_1 = sqrt(error_0_1(:, 2).^2 + error_0_1(:, 3).^2 + error_0_1(:, 4).^2);
error_0_2 = sqrt(error_0_2(:, 2).^2 + error_0_2(:, 3).^2 + error_0_2(:, 4).^2);

time = state_0_0(:, 1);
time = time - time(1);

semilogy(time(10:end-10), error_0_1(10:end-10), time(10:end-10), error_0_2(10:end-10), 'Linewidth', 1.5);
xlabel('Time [s]');
ylabel('Position error [m]');
grid on;
legend('Kepler, 10^{-14}', 'Kepler, 10^{-12}');
title('Benchmark verification (RK7(8) integrator, tol. 10^{-14})');
